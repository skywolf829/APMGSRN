...
class ExpNet(nn.Module):
    ...
    def transform(self, x):
        ...
        transformed_points = (x.unsqueeze(0).repeat(
            self.opt['n_grids'], 1, 1) + 1) / 2
        transformed_points = 2 * (((transformed_points+1)/2) ** (10**self.grid_scales.unsqueeze(1))) - 1
        return transformed_points
    
    def inverse_transform(self, x):
        transformed_points = x.unsqueeze(0).repeat(
            self.opt['n_grids'], 1, 1)
        transformed_points = -1 + 2 * (((transformed_points+1)/2) ** (1/(10**self.grid_scales.unsqueeze(1))))
        return transformed_points

    def fix_params(self):
        with torch.no_grad():
            self.grid_scales.clamp_(-10, 10)
